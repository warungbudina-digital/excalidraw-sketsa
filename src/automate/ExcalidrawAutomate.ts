...
export class ExcalidrawAutomate {
  ...
  // Optional progress sink: each workbench operation reports one stage line (for the UI log).
  private onLog?: (message: string) => void;

  constructor(api: ExcalidrawApi, onLog?: (message: string) => void) {
    this.api = api;
    this.onLog = onLog;
    this.style = { ...DEFAULT_STYLE };
  }

  /** Emit one progress/stage line (no-op when no sink is wired). */
  ...
  addRect(x: number, y: number, width: number, height: number, label?: string): string {
    ...
    this.log(label ? `+ kotak "${label}"` : "+ kotak");
    return id;
  }
  ...
  addEllipse(x: number, y: number, width: number, height: number, label?: string): string {
    ...
    this.log(label ? `+ elips "${label}"` : "+ elips");
    return id;
  }
  ...
  addDiamond(x: number, y: number, width: number, height: number, label?: string): string {
    ...
    this.log(label ? `+ belah ketupat "${label}"` : "+ belah ketupat");
    return id;
  }
  ...
  addText(x: number, y: number, text: string): string {
    ...
    this.log(`+ teks "${text.length > 24 ? `${text.slice(0, 24)}…` : text}"`);
    return id;
  }

  addLine(points: [number, number][]): string {
    const id = this.addLinear("line", points);
    this.log(`+ garis (${points.length} titik)`);
    return id;
  }

  addArrow(points: [number, number][]): string {
    const id = this.addLinear("arrow", points);
    this.log(`+ panah (${points.length} titik)`);
    return id;
  }
  ...
  connect(fromId: string, toId: string, label?: string): string {
    ...
    this.log(label ? `+ panah penghubung "${label}"` : "+ panah penghubung");
    return id;
  }
  ...
  addToGroup(ids: string[]): string {
    ...
    this.log(`+ grup (${ids.length} elemen)`);
    return groupId;
  }
  ...
  addFrame(name: string, childIds: string[]): string {
    ...
    this.log(`+ frame "${name}" (${children.length} anak)`);
    return id;
  }
  ...
  async addMermaid(definition: string): Promise<string[]> {
    ...
    this.log(`+ Mermaid (${converted.length} elemen)`);
  ...
  addRawElements(elements: readonly SceneElement[], files?: SceneFiles): string[] {
    ...
    this.log(`+ ${cloned.length} elemen mentah`);
  ...
  async loadScene(scene: SerializableScene, options: SceneLoadOptions = {}): Promise<boolean> {
    this.clear();
    const ok = await loadSceneIntoApi(this.api, scene, options);
    this.log(`✓ scene dimuat (${scene.elements.length} elemen)`);
    return ok;
  }
  ...
  async addElementsToView(): Promise<boolean> {
    ...
    this.log(`✓ render ${newElements.length} elemen ke kanvas`);
